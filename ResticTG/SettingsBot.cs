...
namespace ResticTG
{
    internal class SettingsBot
    {
        ...
        public string DecryptedString4 { get; private set; }

        private const string RegistryPath = @"SOFTWARE\Restic";
            ...
            try
            {
                using (RegistryKey key = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).OpenSubKey(RegistryPath))
                {
                    if (key != null)
                    {
                        ...
                        string randomServer = (string)key.GetValue("RandomServer", string.Empty);

                        return !string.IsNullOrEmpty(randomUser) &&
                               !string.IsNullOrEmpty(randomPass) &&
                               !string.IsNullOrEmpty(randomName) &&
                               !string.IsNullOrEmpty(randomServer);
                    }
                }
            }
            catch (Exception ex)
        ...
        public void CryptBot(string StringApi, string StringId, string StringName, string ServerName)
        {
            try
            {
                ...
                string encryptedString4 = EncryptString(Environment.MachineName.ToString(), ServerName);
                ...
                using (RegistryKey key = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).CreateSubKey(RegistryPath))
                {
                    if (key != null)
                    {
                        ...
                        key.SetValue("RandomServer", encryptedString4, RegistryValueKind.String);
            ...
            try
            {
                using (RegistryKey key = RegistryKey.OpenBaseKey(RegistryHive.LocalMachine, RegistryView.Registry64).OpenSubKey(RegistryPath))
                {
                    if (key != null)
                    {
                        ...
                        string encryptedString4 = (string)key.GetValue("RandomServer", string.Empty);

                        if (!string.IsNullOrEmpty(encryptedString1) &&
                            !string.IsNullOrEmpty(encryptedString2) &&
                            !string.IsNullOrEmpty(encryptedString3) &&
                            !string.IsNullOrEmpty(encryptedString4))
                        {
                            ...
                            DecryptedString4 = DecryptString(Environment.MachineName.ToString(), encryptedString4);
